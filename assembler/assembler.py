...
def assemble(as_filename, mc_filename):
    ...
    opcodes = [
        ...
        'ior', 'xor', 'rsh', 'lsh', 'rol', 'cmp', 'cpi', 'lia',
        'lir', 'rld', 'rst', 'pld', 'pst', 'mld', 'mst', 'psh',
        'pop', 'mlt', 'mli', 'brh', 'jri', 'jrr', 'cal', 'ret'
    ]    
    ...
    for pc, words in enumerate(instructions):
        ...
        if opcode in [ 'nop', 'hlt', 'inc', 'dec', 'lsh', 'rol', 'not', 'psh', 'pop', 'ret', 'upa' ] and len(words) != 1:
            exit(f'Incorrect number of operands for {opcode} on line {pc}') # 0 operands

        if opcode in [
            'add', 'adi', 'sub', 'and', 'ior', 'xor', 'xor' 'cmp',
            'cpi', 'lia', 'rld', 'rst', 'pld', 'pst', 'mld', 'mst',
            'mlt', 'mli', 'jri', 'jrr', 'cal'
            ] and len(words) != 2:
        ...
        # Reg
        if opcode in ['add', 'sub', 'and', 'ior', 'xor', 'cmp', 'lir', 'rld', 'rst', 'pld', 'pst', 'mld', 'mst', 'mlt', 'jrr']:
            if words[1] != words[1] % (2 ** 3): # 3 bits
                exit(f'Invalid reg for {opcode} on line {pc}')
            machine_code |= words[1] << 8
        
        # Immediates
        if opcode in ['adi', 'cpi', 'lia', 'mli', 'jri']:
        ...
        # RSH
        if opcode in ['rsh']:
            if words[1] != words[1] % (2 ** 1): # 8 bits
                exit(f'Invalid state for {opcode} on line {pc}')
            machine_code |= words[1]
